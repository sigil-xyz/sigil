...
```ts
client.sigilPda(agent: PublicKey): [PublicKey, number]
```

Returns `[pda, bump]` for the Sigil credential of a given agent. Derived from `["sigil", agentPubkey]` on the credential program.
