...
Fetches and decodes the on-chain Sigil account for a given agent.

```ts
const sigil = await client.getSigil(agent: PublicKey): Promise<SigilAccount>
```
...
```ts
await client.recordSpend(agent: PublicKey, amount: BN): Promise<TransactionSignature>
```
