...
<CodeGroup>
```bash npm
npm install @sigil/sdk @coral-xyz/anchor @solana/web3.js bn.js
```

```bash bun
bun add @sigil/sdk @coral-xyz/anchor @solana/web3.js bn.js
```

```bash pnpm
pnpm add @sigil/sdk @coral-xyz/anchor @solana/web3.js bn.js
```
</CodeGroup>

`@sigil/sdk` has peer dependencies on `@coral-xyz/anchor` and `@solana/web3.js` — install them explicitly alongside the SDK.
...
| Import | Use for |
|--------|---------|
| `@sigil/x402` | Express middleware + agent header builder |
| `@sigil/x402/next` | Next.js App Router wrapper |
