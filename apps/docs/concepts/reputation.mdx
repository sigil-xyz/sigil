...
If an agent is reported for malicious behavior, the program authority can slash staked SOL collateral. A slashed agent has its `active` flag cleared and its score reset to 0. (Slashing is currently in development).
